...
class Align_homo():
    ...
    def extract_SURF(self, img):
        ...
        list_kp, desc = surf.detectAndCompute(img_gray, None)
    ...
    def match_SURF(self, desc_s, desc_t):
    ...
    def align_image(self):
        ...
        ## 提取特征
        self.tik = time.time()
        kp_s, desc_s, list_kp_s = self.extract_SURF(img_source)
        kp_t, desc_t, list_kp_t = self.extract_SURF(img_target)
        self.tok = time.time()
        extract_time = self.tok - self.tik
        ## 特征点匹配  对应索引array，[[最近的点]]
        self.tik = time.time()
        fit_pos, good_matches = self.match_SURF(desc_s, desc_t)
        self.tok = time.time()
