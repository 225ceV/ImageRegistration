...
class Align_homo():
    ...
    def extract_orb(self, img):
        ...
        list_kp = orb.detect(img_gray, None)
        list_kp, desc = orb.compute(img, list_kp)
        # Extract positions of key points
        kp = np.array([p.pt for p in list_kp]).T

        return kp, desc, list_kp

    def match_orb(self, desc_s, desc_t):
    ...
    def align_image(self):
        ...
        ## 特征提取
        self.tik = time.time()
        kp_s, desc_s, list_kp_s = self.extract_orb(img_source)
        kp_t, desc_t, list_kp_t = self.extract_orb(img_target)
        self.tok = time.time()
        extract_time = self.tok - self.tik
        # 特征点匹配  对应索引array，[[最近的点]]
        self.tik = time.time()
        fit_pos, good_matches = self.match_orb(desc_s, desc_t)
        self.tok = time.time()
